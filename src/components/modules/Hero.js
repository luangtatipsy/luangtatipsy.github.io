...
const Avatar = () => {
  return (
    <div className="rounded-full flex items-center justify-center w-32 h-32 bg-gray-200">
      <img className="w-20" src={avatar} alt="My memoji avatar" />
    </div>
  );
};
  ...
  return (
    <div className="container mx-auto pt-16 pb-32 bg-transparent flex flex-col items-center justify-center">
      ...
      <div className="content-wrapper px-8 sm:px-auto mt-8 text-center">
        ...
        <h1 className="hidden sm:block text-4xl sm:text-5xl font-bold py-7 uppercase tracking-wider">
          {profile.position.full}
        </h1>
        <h1 className="block sm:hidden text-4xl sm:text-5xl font-bold py-7 uppercase tracking-wider">
          {profile.position.short}
        </h1>
        {profile.tagLine.split("\n").map((line, idx) => {
          return (
            <p key={idx} className="text-xl font-extralight tracking-wider">
              {line}
            </p>
          );
        })}
